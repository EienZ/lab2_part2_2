...
def create_instance(image_id, min_count, max_count, instance_type, key_name, region):
    ec2_client = boto3.client("ec2", region_name=region)
    ...
    print(f"Екземпляр створено з ID: {instance_id}")
...
while True:
    ...
    if choice == "1":
        ...
        min_count = int(input("Мінімальна кількість екземплярів: "))
        max_count = int(input("Максимальна кількість екземплярів: "))
        instance_type = input("Тип екземпляру (наприклад, t4g.nano): ")
        key_name = input("Назва ключової пари: ")
        create_instance(image_id, min_count, max_count, instance_type, key_name, region)
    elif choice == "3":
